...
| OpenCode | `.opencode/plugins/ponytail.mjs`, `.opencode/commands/`, `hooks/`, `skills/` | One package supports OpenCode 2.0.15+ through `Plugin.define({ id, setup })` and OpenCode 1.18.29+ through `server()`. Both adapters register commands and skills, inject the active mode into agent-loop requests, and share the user-global mode file. |
...
| VS Code + Codex extension | `AGENTS.md` | The Codex extension reads `AGENTS.md` (repo root, or `~/.codex/AGENTS.md` globally). Instruction-tier; the full Codex plugin row above adds `/ponytail` modes and hooks. |
